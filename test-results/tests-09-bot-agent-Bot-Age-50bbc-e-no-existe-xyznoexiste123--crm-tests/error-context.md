...
- Name: tests/09-bot-agent.spec.ts >> Bot Agent — Evaluación funcional >> [B01] busqueda: "Busca algo que no existe xyznoexiste123"
- Location: tests/tests/09-bot-agent.spec.ts:196:9

# Error details

```
Error: Esperaba patrón /no.*encontr|sin resultado|no hay/i en la respuesta del bot

expect(received).toMatch(expected)

Expected pattern: /no.*encontr|sin resultado|no hay/i
